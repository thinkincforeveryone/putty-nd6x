...
//        TcpClient* theClient);
    ...
    inline bool isClose()
    {
        return statusM == CloseE;
    }
    inline bool isRBufferHealthy()
    {
        return inputQueueM.isHealthy();
    };
    inline bool isWBufferHealthy()
    {
        return outputQueueM.isHealthy();
    };
    unsigned getRBufferSize()
    {
        return inputQueueM.size();
    };
    unsigned getWBufferSize()
    {
        return outputQueueM.size();
    };
    ...
    void setPeerAddr(const struct sockaddr_in* theAddr)
    {
        peerAddrM = *theAddr;
    }
    const struct sockaddr_in& getPeerAddr()
    {
        return peerAddrM;
    }

    //heartbeatTimeoutCounterM
    void resetHeartbeatTimeoutCounter()
    {
        heartbeatTimeoutCounterM = 0;
    }
    int incHeartbeatTimeoutCounter()
    {
        return heartbeatTimeoutCounterM++;
    }
    int getHeartbeatTimeoutCounter()
    {
        return heartbeatTimeoutCounterM;
    }
    ...
    void setUpperData(void* theUpperData)
    {
        uppperDataM = theUpperData;
    }
