...
public:
    ...
    const char* curBuffer()
    {
        return buffer_.c_str()+decodeIndex_;
    }
    void eatBuffer()
    {
    ...
    bool isDoingRz()
    {
        return (getCurState().getId() !=  IDLE_STATE) || bufferParsed_;
    };
    int lengthToBeDecode()
    {
        return buffer_.length() - decodeIndex_;
    };
    const char* bufferToBeDecode()
    {
        return buffer_.c_str() + decodeIndex_;
    }
