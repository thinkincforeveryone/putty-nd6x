...
static void create_controls(HWND hwnd, char * _path)
{
	...
	for (index = -1; (index = ctrl_find_path(ctrlbox, path, index)) >= 0;)
	{
		...
		if (s->pop_pos && !pos_stack.empty())
		{
			pos_stack.pop_back();
		}
		if (s->push_pos)
		{
			pos_stack.push_back(cp);
		}
		if (s->use_pos && !pos_stack.empty())
		{
			cp = pos_stack[pos_stack.size() - 1];
		}
...
static int edit_session_treeview(HWND hwndSess, int eflag)
{
	...
	case EDIT_BEGIN:
		...
		if (dragging)
		{
			drag_session_treeview(GetDlgItem(hwndSess, IDCX_SESSIONTREEVIEW), DRAG_CANCEL, 0, 0);
		}
...
static void refresh_tree_view(const char* old_session, const char* new_session, HWND treeview = NULL)
{
	...
	if (treeview == NULL)
	{
		treeview = GetDlgItem(hConfigWnd, IDCX_TREEVIEW);
	}
