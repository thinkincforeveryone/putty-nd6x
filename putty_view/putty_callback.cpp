...
void process_init()
{
    ...
    conf_set_int(cfg, CONF_logtype, LGTYP_DEBUG);
	...
	AppendMenu(m, MF_SEPARATOR, 0, 0);
	AppendMenu(m, MF_ENABLED, IDM_SFTP, TEXT("Connect SFTP..."));
...
Context get_ctx(void *frontend)
{
    ...
    {
        puttyController->m_PuttyHdc = GetDC(puttyController->getNativePage());
        if (puttyController->m_PuttyHdc && puttyController->pal)
        {
            SelectPalette(puttyController->m_PuttyHdc, puttyController->pal, FALSE);
        }
    }
...
void free_ctx(void *frontend, Context ctx)
{
    ...
    NativePuttyController *puttyController = (NativePuttyController *)frontend;
    if (puttyController->m_PuttyHdc)
    {
        SelectPalette(puttyController->m_PuttyHdc, (HPALETTE__*)GetStockObject(DEFAULT_PALETTE), FALSE);
        ReleaseDC(puttyController->getNativePage(), puttyController->m_PuttyHdc);
        puttyController->m_PuttyHdc = NULL;
    }
}
               ...
               unsigned long attr, int lattr)
{
    ...
    int fnt_width;
    int char_width;
    HDC hdc = puttyController->m_PuttyHdc;
...
void palette_reset(void *frontend)
{
    ...
    if (puttyController->pal)
    {
        ...
        get_ctx(frontend);
        hdc = puttyController->m_PuttyHdc;
        assert (hdc != NULL);
        RealizePalette(hdc);
        free_ctx(frontend, (Context)frontend);
    }
