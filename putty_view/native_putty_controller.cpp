...
int NativePuttyController::on_paint()
{
	...
	m_PuttyHdc = BeginPaint(page_->getWinHandler(), &p);
    if (pal)
    {
        SelectPalette(m_PuttyHdc, pal, TRUE);
        RealizePalette(m_PuttyHdc);
    }
            ...
            p.rcPaint.bottom>= offset_height + font_height*term->rows)
    {
        ...
        oldbrush = (HBRUSH__*)SelectObject(m_PuttyHdc, fillcolour);
        edge = CreatePen(PS_SOLID, 0,
                         colours[ATTR_DEFBG>>ATTR_BGSHIFT]);
        oldpen = (HPEN__*)SelectObject(m_PuttyHdc, edge);
         ...
         */
        IntersectClipRect(m_PuttyHdc,
                          p.rcPaint.left, p.rcPaint.top,
                          p.rcPaint.right, p.rcPaint.bottom);

        ExcludeClipRect(m_PuttyHdc,
                        offset_width, offset_height,
                        offset_width+font_width*term->cols,
                        offset_height+font_height*term->rows);

        Rectangle(m_PuttyHdc, p.rcPaint.left, p.rcPaint.top,
                  p.rcPaint.right, p.rcPaint.bottom);

        /* SelectClipRgn(m_PuttyHdc, NULL); */

        SelectObject(m_PuttyHdc, oldbrush);
        DeleteObject(fillcolour);
        SelectObject(m_PuttyHdc, oldpen);
        DeleteObject(edge);
    }
    SelectObject(m_PuttyHdc, GetStockObject(SYSTEM_FONT));
    SelectObject(m_PuttyHdc, GetStockObject(WHITE_PEN));
                                    ...
                                    WPARAM wParam, LPARAM lParam)
{
    ...
    case IDM_DUPSESS:
        ...
        break;
	case IDM_SFTP:
		WindowInterface::GetInstance()->sftpCurSession();
		break;
    case IDM_RECONF:
        on_reconfig();
        break;
        ...
        unsigned long attr, int lattr)
{
    ...
    fg = colours[nfg];
    bg = colours[nbg];
    SelectObject(m_PuttyHdc, fonts[nfont]);
    SetTextColor(m_PuttyHdc, fg);
    SetBkColor(m_PuttyHdc, bg);
    if (attr & TATTR_COMBINING)
        SetBkMode(m_PuttyHdc, TRANSPARENT);
    else
        SetBkMode(m_PuttyHdc, OPAQUE);
    line_box.left = x;
    line_box.top = y;
    ...
    if (font_varpitch)
    {
        ...
        SetTextAlign(m_PuttyHdc, TA_TOP | TA_CENTER | TA_NOUPDATECP);
        lpDx_maybe = NULL;
        maxlen = 1;
    }
    else
    {
         ...
         */
        xoffset = 0;
        SetTextAlign(m_PuttyHdc, TA_TOP | TA_LEFT | TA_NOUPDATECP);
        lpDx_maybe = lpDx;
        maxlen = len;
    }
            ...
            text += len, remaining -= len, x += char_width * len)
    {
        ...
        if (ucsdata.dbcs_screenfont && (text[0] & CSET_MASK) == CSET_ACP)
        {
            ...
            ExtTextOutW(m_PuttyHdc, x + xoffset,
            ...
            {
                SetBkMode(m_PuttyHdc, TRANSPARENT);
                ExtTextOutW(m_PuttyHdc, x + xoffset - 1,
                            y - font_height * (lattr ==
        ...
        else if (DIRECT_FONT(text[0]))
        {
            ...
            ExtTextOut(m_PuttyHdc, x + xoffset,
                       y - font_height * (lattr == LATTR_BOT) + text_adjust,
                       ETO_CLIPPED | (opaque ? ETO_OPAQUE : 0),
            ...
            if (bold_font_mode == BOLD_SHADOW && (attr & ATTR_BOLD))
            {
                 ...
                 * column is blank...
                 */
                ExtTextOut(m_PuttyHdc, x + xoffset - 1,
                           y - font_height * (lattr ==
                                              LATTR_BOT) + text_adjust,
                                ...
                                && (attr & ATTR_UNDER))))
    {
        ...
        oldpen = (HPEN__*)SelectObject(m_PuttyHdc, CreatePen(PS_SOLID, 0, fg));
        MoveToEx(m_PuttyHdc, x, y + dec, NULL);
        LineTo(m_PuttyHdc, x + len * char_width, y + dec);
        oldpen = (HPEN__*)SelectObject(m_PuttyHdc, oldpen);
        DeleteObject(oldpen);
    }
}
        ...
        CONST INT *lpDx, int opaque)
{
    ...
    for (i = 0; i < (int)cbCount ;)
    {
        ...
        if (rtl)
        {
            exact_textout(m_PuttyHdc, xp, y, lprc, lpString+i, j-i,
                          font_varpitch ? NULL : lpDx+i, opaque);
        }
        else
        {
            ExtTextOutW(m_PuttyHdc, xp, y, ETO_CLIPPED | (opaque ? ETO_OPAQUE : 0),
                                        ...
                                        WPARAM wParam, LPARAM lParam)
{
    if (message == wm_mousewheel || message == WM_MOUSEWHEEL)
    {
        ...
        while (abs(wheel_accumulator) >= WHEEL_DELTA)
        {
            ...
            {
                if (font_height_by_wheel == 0)
                {
                    font_height_by_wheel = font_height;
                }
                font_height_by_wheel = b == MBT_WHEEL_UP ? font_height_by_wheel + 1 : font_height_by_wheel - 1;
                if (font_height_by_wheel < 6)
                {
                    font_height_by_wheel = 6;
                }
                if (font_height_by_wheel > 60)
                {
                    font_height_by_wheel = 60;
                }
                char tips[64] = { 0 };
        ...
        WPARAM wParam, LPARAM lParam)
{
    if ((HWND) wParam != hwnd && pal != NULL)
    {
        ...
        if (item && item->m_PuttyHdc)
        {
            if (RealizePalette(item->m_PuttyHdc) > 0)
                UpdateColors(item->m_PuttyHdc);
