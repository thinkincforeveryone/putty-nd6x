                 ...
                 int w, int h, int ps, char *basefilename, int subsamp, int qual, int flags)
{
    ...
    if(flags&TJ_BGR)
    {
        ...
        {
            if(flags&TJ_ALPHAFIRST) pixformat="ABGR";
            else pixformat="BGRA";
        }
    }
    else
    {
        ...
        {
            if(flags&TJ_ALPHAFIRST) pixformat="ARGB";
            else pixformat="RGBA";
        }
    }
...
{
    tjhandle hnd=NULL, dhnd=NULL;
    unsigned char *jpegbuf=NULL;
    unsigned long size;

    if((jpegbuf=(unsigned char *)malloc(TJBUFSIZE(w, h))) == NULL)
    {
        puts("ERROR: Could not allocate buffer.");
        bailout();
    }
...
{
    int i, j, i2;
    unsigned char *bmpbuf=NULL, *jpgbuf=NULL;
    tjhandle hnd=NULL;
    unsigned long size;
    if((hnd=tjInitCompress())==NULL)
    {
