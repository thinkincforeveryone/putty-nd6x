...
void Browser::NewTab()
{
    ...
    {
        AddBlankTab(true);
        GetSelectedTabContentsWrapper()->tab_contents()->GetView()->RequestFocus();
    }
    else
    {
        ...
        b->GetSelectedTabContentsWrapper()->tab_contents()->GetView()->RequestFocus();
    ...
    int id, WindowOpenDisposition disposition)
{
    ...
    case IDC_NEW_TAB:
        AddBlankTab(true);
        break;
    case IDC_SESSION_SETTING:
        if (tab) tab->do_reconfig();;
        break;
    case IDC_COPY_ALL:
        if (tab) tab->do_copyAll();;
        break;
    case IDC_CLEAR:
        if (tab) tab->do_clearScrollbar();;
        break;
    case IDC_LOG_ENABLER:
        if (tab) tab->do_log(!tab->isLogStarted());;
        break;
    case IDC_SHORTCUT_ENABLER:
        if (tab) tab->do_shortcutEnabler(!tab->isShortcutEnabled());;
        break;
    case IDC_SCROLL_TO_END:
        if (tab) tab->setScrollToEnd(!tab->getScrollToEnd());;
        break;
    case IDC_CMD_DLG:
        if (tab) tab->notifyMsg("show_cmd_dlg", NULL);;
        break;
    ...
    case IDC_FIND_NEXT:
        if (tab) tab->searchNext(searchStr);
        break;
    case IDC_FIND_PREVIOUS:
        if (tab) tab->searchPrevious(searchStr);
        break;
    case IDC_FIND_RESET:
        if (tab) tab->resetSearch();
        break;
    ...
    case IDC_ABOUT:
        showabout(window()?window()->GetNativeHandle():NULL);
        break;
...
TabContentsWrapper*  Browser::sftpCurSession()
{
	base::TimeTicks new_tab_start_time = base::TimeTicks::Now();
	browser::NavigateParams params(this, Url::EmptyGURL()/*GURL(chrome::kChromeUINewTabURL)*/);
	params.disposition = NEW_FOREGROUND_TAB;
	...
	{
		AddTab(new TabContentsWrapper(new TabSftpContents(-2, NULL)));
	}
    ...
    bool isWithGlobalCfg)
{
    ...
    TabContents* new_contents = new TabPuttyContents(routing_id, base_tab_contents);
