...
void TabContents::SetView(view::View *view)
{
	m_view = view;
}

view::View* TabContents::GetView() const
{
	return m_view;
}
...
{
	//return m_view->do_shortcutEnabler(isPressed);
}
...
{
	//return m_view->cmdScat(type, buffer, buflen, interactive);
}

void TabContents::sendScript(int type, const char * buffer, int buflen, int interactive) const
{
	//return m_view->sendScript(type, buffer, buflen, interactive);
}

Conf* TabContents::getCfg()
{
	//return m_view->getCfg();
	return NULL;
}
...
{
	return true; /*|| (web_ui() && web_ui()->IsLoading())*/
}
